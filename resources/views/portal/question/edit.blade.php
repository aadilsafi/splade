...
@endif
	<form method="post" action="{{route('update-question',$question->id)}}" enctype="multipart/form-data" id="questionForm">
		@csrf
		@method('PUT')
		<div class="form-group row offset-md-1">
			<div class="col-md-10">
				<label for="#description">Question</label>
				<textarea type="text" name="description"  class="form-control" id="description">{{$question->description}}</textarea>
			</div>
		</div> <br><br>
		<div class="form-group row offset-md-1">
			<div class="col-md-5">
			...
			<div class="col-md-5">
				<select name="difficulty_level" id="difficulty_level" class="form-control ">
					...
					@foreach($difficulty_levels as $key => $difficulty_level)
						<option value="{{$key}}"
						@if($key == $question->difficulty_level)
							{{'selected'}}
						@endif
		...
		<div class="form-group row offset-md-1 my-3" id="choices">
		@foreach($answers as $key => $answer)
			<div class='d-flex common_value' id='{{'answer_no_'.($key+1)}}'>
				<div class='col-md-4 mt-3'>
					<input type='text' class='form-control' placeholder='Multiple Choice' name='choices[]' value="{{$answer->title}}">
				</div>
				<div class='col-md-4 mt-3'>
					<input type='text' class='form-control'
						placeholder='Description... (optional)' 
						name='descriptions[]' value="{{$answer->description}}">
				</div>
				<div class='col-md-3 mt-3 custom-control custom-radio d-flex choice_parent'>
					<input type="radio" id="{{'correct_'.($key+1)}}" name="correct" class="custom-control-input choice_child" value='{{($key+1)}}'

					@if($answer->is_correct == 1)
						{{'checked'}}
					@endif
					>
					<label for='{{'correct_'.($key+1)}}' class='custom-control-label choice_label'>Correct Answer</label>

					<div style='margin-top:-20px;margin-left:10px;cursor:pointer'
					 onclick="{{"removeAnswer("."'answer_no_".($key+1)."',".($key+1).')'}}">
		...
		<br><br>
		<input type="submit" class="btn btn-success my-3" style="float: right;margin-right: 200px;" value="Update Question">
...
<script>
	...
	$(document).ready(function(){
		let choice_number = $('#choices').children().length;
		choice_number++;
